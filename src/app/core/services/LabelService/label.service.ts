...
export class LabelService {
  public API = '//localhost:8080/label';
  ...
  public deleteLabel(label) {
    ...
    return this.http.deleteWithParams(this.API + '/delete/' + token, {
      params: {
        labelName: labelName
      }
    });
  ...
  public updateLabel(label) {
    var header = this.getHeader();
    return this.http.put(this.API + '/update', label, header);
  }

  public createLabel(label){
    var header = this.getHeader();
    return this.http.postWithBody(this.API + '/create', label, header);
  }
